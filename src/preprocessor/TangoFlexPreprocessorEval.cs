...
public class TangoFlexPreprocessorEval
{
    ...
    public TangoFlexPreprocessorEval(Token[] tokens, Arguments arguments)
    {
        this.tokens = tokens;
        tokenOut = Process();
    ...
    private bool EvaluateConditional(PreprocessorVal type)
    {
        ...
        while (TokensIndex < tokens.Length && Peek().Kind != TokenKind.NEWLINE)
    ...
    Token Peek(int o = 0) => TokensIndex + o < tokens.Length ? tokens[TokensIndex + o] : null;
